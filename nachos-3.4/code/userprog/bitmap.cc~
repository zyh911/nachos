...
{ 
    ...
    myEntry = new TranslationEntry*[nitems];
...
BitMap::findPage()
{
	int mymin = 99999999, returnValue = -1;;
	...
	for (int i = 0; i < numBits; i++) {
		...
		if(myEntry[i] != NULL && myEntry[i]->lastUsedTime < mymin) {
			mymin = myEntry[i]->lastUsedTime;
			returnValue = i;
		}
	}
	return returnValue;
