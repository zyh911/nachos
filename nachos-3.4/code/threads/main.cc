...
void havefun()
{
	printf("------------------\nthe left threads' names are:\n");
	for(int i = 0; i < 128; i ++)
	{
		if(a[i].b == true)
		{
			//printf("%drrrrrrrrrrrrrrrrr\n", i);
			printf("%s\n", (a[i].p)->getName());
		}
	}
	printf("------------------\n");
...
*/
	...
	if (!strcmp(*argv, "ts"))
	    ...
	    ASSERT(argc > 1);
            StartProcess(*(argv + 1));
            argCount = 2;
        } else if(!strcmp(*argv, "-users")) {
                printf("Running ThreadTests() on -users mode\n");
                ThreadTest();
                break;
	...
	if (!strcmp(*argv, "-cp")) { 		// copy from UNIX to Nachos
	    ...
	    argCount = 2;
            ...
            PerformanceTest();
	} else if (!strcmp(*argv, "-mkdir")) {	// mkdir a directory file
	    ASSERT(argc > 1);
	    Mkdir(*(argv + 1));
	    argCount = 2;
	    ...
	    Cd(*(argv + 1));
	    argCount = 2;
	} else if (!strcmp(*argv, "-pwd")) {	// show current path
            fileSystem->ShowPath();
	    printf("\n\n");
	}
#endif // FILESYS
