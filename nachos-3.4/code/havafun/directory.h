...
class DirectoryEntry {
  public:
    ...
    char name[FileNameMaxLen];		// Text name for file, with +1 for 
					// the trailing '\0'

    int fileType;			//for directory:0, for data file:1
    int dateCreated;	                //time created
    int lastReferenced;			//time last opened
    int lastModified;			//time last modified
    char* path;				//path in the directory
    int FID;				//unique file identifier
};
...
class Directory {
  public:
    ...
    void setDataCreated(char* name);
    void updateLastReferenced(char* name);
    bool updateLastModified(char* name);
    int getFID(char *name);
