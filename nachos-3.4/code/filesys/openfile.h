...
#define StringMaxLen		12	// for strings length
...
class OpenFile {
  public:
    ...
    int SysCallRead(char *into, int numBytes); // Read/write bytes from the file,only for syscall
    int Write(char *from, int numBytes);
    int SysCallWrite(char *from, int numBytes);//only for syscall

    int ReadAt(char *into, int numBytes, int position);
    					// Read/write bytes from the file,
					// bypassing the implicit position.
    int SysCallReadAt(char *into, int numBytes, int position);
    int WriteAt(char *from, int numBytes, int position);
    int SysCallWriteAt(char *from, int numBytes, int position);

    int Length(); 			// Return the number of bytes in the
					// file (this interface is simpler 
					// than the UNIX idiom -- lseek to 
					// end of file, tell, lseek back 
    void IncreaseLength(int Bytes);	// Increase the file length on hdr->numBytes   
    //Edit by YePeng-------------
...
//  private:
    ...
    int currentSector;			// Current file header's sector on the disk --edit by YePeng
};
